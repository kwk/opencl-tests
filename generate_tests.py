...
def generate_function_test(func_data, category):
    ...
    code.append("    cl_program program = clCreateProgramWithSource(context, 1, &kernelSourcePtr, &kernelSize, &err);")
    ...
    code.append("        clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_LOG, sizeof(buildLog), buildLog, NULL);")
    ...
    for i in range(num_inputs):
        ...
        code.append(f"    cl_mem inputBuffer{i} = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,")
        code.append(f"                                        NUM_TESTS * sizeof({buffer_type}), input{i}, &err);")

    code.append(f"    cl_mem outputBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY,")
    code.append(f"                                         NUM_TESTS * sizeof({output_c_type}), NULL, &err);")
    code.append("")
    ...
    code.append("    err = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &globalSize, NULL, 0, NULL, NULL);")
    code.append(f"    clEnqueueReadBuffer(queue, outputBuffer, CL_TRUE, 0, NUM_TESTS * sizeof({output_c_type}), output, 0, NULL, NULL);")
    code.append("")
    ...
    if is_vector_type(output_type):
        ...
        if 'float' in output_type:
            ...
            if output_type.endswith('2'):
                code.append("        bool passed = floatEquals(output[i].s[0], expected[i].s[0]) &&")
